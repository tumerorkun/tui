...
import { calculateStrokeWidth } from "../utils";
import animations from "../styles/animations.modules.scss";
console.log(animations);
export type SvgProps = React.SVGProps<SVGSVGElement> & {
  className?: string;
  primaryColor?: string | "currentColor";
  width?: number | "100%";
  height?: number | "100%";
  size?: number | "100%";
...
export const Reload = forwardRef<SVGSVGElement, SvgProps>(
  (
    {
      primaryColor,
      width = 24,
    ...
    return (
      <svg
        ...
        fill="none"
        {...others}
        className={`icon ${others.className ?? ""}`.trim()}
      >
        <path
          ...
          stroke={primaryColor}
          strokeWidth={calculateStrokeWidth(size ?? strokeWidthFactor, width)}
          strokeLinecap="round"
...
export const AnimatedReload = forwardRef<SVGSVGElement, SvgProps>(
  ({ className, ...props }, ref) => {
    return (
      <Reload
        {...props}
        ref={ref}
        className={`${className} ${animations.rotate_infinity}`.trim()}
      />
    );
  }
);
